...
namespace SequenceOfOperators
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            Console.WriteLine(" Binary representation of your number: {0}, decimal: {1}\n", Convert.ToString(number, 2), number);

            int mask = 1 << position;
            number = (v == 0) ? number & ~mask : number | mask;
            Console.WriteLine(" Binary representation of your number after modification: {0}, decimal: {1}\n", Convert.ToString(number, 2), number);
        }
    }
}
