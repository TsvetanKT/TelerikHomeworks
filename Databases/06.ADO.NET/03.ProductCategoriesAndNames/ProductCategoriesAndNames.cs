namespace ProductCategoriesAndNames
{
    using System;
    using System.Data.SqlClient;

    class ProductCategoriesAndNames
    {
        ...
        static void Main()
        {
            ...
            using (dbCon)
            {
                SqlCommand cmdProductCategoriesAndNames = new SqlCommand(
                "SELECT c.CategoryName, p.ProductName FROM Categories c " +
                "JOIN Products p " +
                "ON c.CategoryID = p.CategoryID " +
                "GROUP BY c.CategoryName, p.ProductName", dbCon);

                SqlDataReader reader = cmdProductCategoriesAndNames.ExecuteReader();
